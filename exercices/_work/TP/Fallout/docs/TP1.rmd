...
7. Implémenter un test unitaire :

Modifiez le fichier `src/test/java/cpe/[votrenom]/AppTest.java`

```java	
public class AppTest {
    @Test public void personnageName() {
        try {
            new PersonnageRPG("Jo");
        } catch (IllegalArgumentException e) {
            System.err.println(e.getMessage());
            System.out.println("Test réussi");
            assertEquals("Le nom doit contenir au moins 3 caractères", e.getMessage());
        } 
       new PersonnageRPG("...");
    }
}
...
8. Vous pouvez lancer vos tests depuis ou l'IDE ou en ligne de commande avec `gradle test`.

> **Lorsque vous commitez, les tests sont automatiquement lancés et vous recevez un mail si vos tests ne passent pas, vous devez coder un nombre de tests assez raisonnable**
...
Rajouter des tests unitaires, ne prévoyez pas que le fonctionnement nominal, vous devez aussi écrire des tests censés planter, grâce au try catch vous pourrez vérifier que les exceptions sont bien levées.
