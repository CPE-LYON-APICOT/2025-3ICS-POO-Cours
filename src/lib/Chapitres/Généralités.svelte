...
<Slide>
	...
	<Slide>
		<p>
			...
			Un objet est une instance d'une classe, regroupant des données (attributs) et des comportements (méthodes).
		</p>
	</Slide>
	<Slide>
		<p>Pour créer un objet, on utilise le mot-clé <b>new</b> suivi du nom de la classe.</p>
			<p>
				Chaque classe possède un constructeur par défaut qui peut être redéfini afin de personnaliser la création de l'objet.
			</p>
		<Code>
			{`
...
`}
	...
	<Slide>
		<p>
			Il peut y avoir plusieurs constructeurs dans une classe, on parle de surcharge de constructeurs.
		</p>
		<Code>
			{`
			public class Personne {
...
`}
	...
	<Slide>
		<p>
			Les modificateurs d'accès permettent de définir la visibilité des attributs et des méthodes.
		</p>
		<p>Il existe 4 modificateurs d'accès :</p>
		<ul>
			<li><code>public</code> : accessible partout</li>
			<li><code>protected</code> : accessible dans la classe et dans ses sous-classes</li>
			<li><code>private</code> : accessible uniquement dans la classe</li>
			<li><code>default</code> : accessible uniquement dans le package</li>
		</ul>
	...
	<Slide>
		<h3>Gestion des erreurs</h3>
			...
			<p>
				En Java, on utilise les blocs <code>try</code>, <code>catch</code> et <code>finally</code> pour gérer les exceptions.
			</p>
			<Code>
				{`
				try {
